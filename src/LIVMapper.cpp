...
void LIVMapper::initializeComponents() 
{
  ...
  if (!vk::camera_loader::loadFromRosNs("fast_livo2", vio_manager->cam)) throw std::runtime_error("Camera model not correctly specified.");
...
void LIVMapper::handleVIO() 
{
  ...
  ROS_DEBUG("[VIO] raw feature num=%zu", pcl_w_wait_pub->points.size());
...
#endif
  for (int i = 0; i < static_cast<int>(world_lidar->points.size()); ++i)
  {
